...
- `Studio App/updater.py` - Self-update from the PRIVATE `Willzy14/StemToAbleton-Releases` GitHub repo via a baked read-only PAT. `_api_get`/`_fetch` (2026-07-29): retries once through a `certifi`-backed SSL context if the host machine's own certificate store can't verify GitHub's connection (narrowly on `ssl.SSLCertVerificationError`, not any SSL error). `write_swap_script` (2026-07-29): the swap runs in a VISIBLE console (not detached — a silent multi-second wait looked exactly like a broken update), retry capped at 30s via a console-safe `ping` wait (not `timeout`, which errors with no console attached), and writes a result marker (`%APPDATA%\StemToAbleton\update_ok.txt` / `update_failed.txt`) that `pop_update_result()` reads once so the app can report the real outcome on its next launch.
- `Studio App/build_exe.py` - Builds the onefile PyInstaller EXE and bakes in the release token. The final copy-into-place step (2026-07-29) verifies the destination's sha256 actually matches what was just built before reporting success — a silent copy failure here once shipped a stale binary under a new version number with no error at all (see Snag List SNAG-005). Never trust this script's own "Built:" message without also checking the actual published/`dist/` file if it matters (release day).
