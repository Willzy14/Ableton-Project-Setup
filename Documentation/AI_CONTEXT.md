...
1. **#1 TOMORROW — multi-version later sections (radio edit) land OFF the bar grid.** Sam confirmed on Fallon V2: the radio kick is not on grid. DIAGNOSED 2026-06-25: (a) the radio kick stem is silent until ~31.4 s (first region 31.4–75.4 s), but `detect_bpm` returns `first_beat_sec=0.034` — the *extrapolated grid phase folded to one beat*, NOT the first actual kick at 31.4 s — so `build_multiversion`'s `base_offset = bar − first_beat·k` shifts by ~0 instead of ~31.4 s and the whole section sits ~0.07 beat off the bar. FIX: align by the FIRST ACTUAL KICK ONSET (kick stem's first region start / first picked onset), not the folded lattice phase — return that from `detect_bpm` (or compute in build_multiversion) and set `base_offset = bar − (first_kick_sec/60)·bpm`. (b) Secondary: detected BPM = 127.71 but project tempo is rounded to 128, so unwarped clips DRIFT against the grid across a section (~1.5 beats over 6 min). Consider setting the project tempo to the precise detected BPM (no-warp means rounding drifts), or discuss warping with Sam. The EXTENDED is on-grid only because its kick is at t≈0 (no accumulated drift).
...
4. **Python 3.14.0 interpreter instability** — intermittent AND sometimes persistent miscompiles of hot pure-Python loops (bogus TypeError / segfault / "int not iterator"). Hot paths (bounce, find_audio_regions) numpy-ified to dodge it; `bpm_detector` still has stdlib loops that can glitch. **Sam to install stable Python 3.13 (first thing tomorrow)** — removes it entirely; run with `PYTHON_JIT=0` meanwhile.

## What's Next
1. **Install Python 3.13** (kills the 3.14 glitches; code runs identically).
2. **Fix multi-version off-grid** (known-issue #1 above) — align sections by the first actual kick onset; address the BPM-rounding drift.
3. **Same-folder name-token versions** — Get Right `S16` vs `S17 -SHRT EDIT` (reuses the multi-version engine; extend `versions.detect_versions` to split a single folder's mirror sets by a name token).
4. **Wet/dry** — wet ON (normal tracks); dry grouped + muted underneath.
