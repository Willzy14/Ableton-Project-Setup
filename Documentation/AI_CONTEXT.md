...
3. **Package + auto-update the Studio App** (model DECIDED — see Current State): PyInstaller EXE (ML-off), a `latest.json` updater that self-swaps, and a public "releases" feed + publish script (code stays private). Repoint the `⟳ Update` button from `git pull` to the `latest.json` feed.
