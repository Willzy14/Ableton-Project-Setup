...
2. **Build nested sub-groups** (spec LOCKED — item 5 below). Biggest engine feature left. Singer-first; vocals + offered for drums/music. Layered build: nested GroupTracks in `als_patcher` → name-clustering helper → layout in `project_builder` → synthetic nested-ALS validation.
