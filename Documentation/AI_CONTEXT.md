...
### ⭐ Tomorrow (2026-06-30) — priority order
1. **Sam: run the Studio App** — `py -3.13 -m pip install pywebview` then `Studio App/Run Studio App.bat`. Feed back on look/feel + the colour-profile flow. (Claude can't test the native window headless.)
2. ~~**Build nested sub-groups**~~ **DONE (2026-06-30)** — see Current State. `Source/subgroup_cluster.py` + `als_patcher._apply_track_groups` (nested GroupTracks) + `build_project(subgroup_categories=...)`; validated end-to-end (`Tests/test_subgroups.py` 11/11). Follow-ups: apply to the multi-version path; surface a UI toggle.
...
4. **Studio App real OS drag-drop** of file paths (currently click-to-browse; drag falls back to picker).
5. **ML subprocess cleanup/timeouts** (Codex handoff list — `Documentation/CODEBASE_REVIEW_NEXT_STEPS.md`).
6. **Logo / branding pass** on the Studio App (Sam wants it sleek; he did the Leveller branding fast).
