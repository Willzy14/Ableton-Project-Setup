...
def test_multiversion_unknown_stems_use_ml_category():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        project_builder.find_audio_regions = lambda _path, head_sec=0.0, return_peak=False: (([(0.0, 1.0)], -3.0) if return_peak else [(0.0, 1.0)])
        project_builder.find_group_buses = lambda _paths: []
        try:
            mix, refs, buses, _dry, _skipped = project_builder._process_version_files(
                [source],
                output_dir,
                "Audio/Extended/",
            )
...
def test_multiversion_unknown_stems_fall_back_to_music_when_ml_disabled():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        project_builder.find_group_buses = lambda _paths: []
        try:
            mix, _refs, _buses, _dry, _skipped = project_builder._process_version_files(
                [source],
                root / "Output Audio",
                "Audio/Extended/",
                use_ml=False,
